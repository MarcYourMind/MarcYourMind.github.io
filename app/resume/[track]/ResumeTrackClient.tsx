                ...
                <div className="container mx-auto px-6 max-w-5xl">
                    <header className="mb-16 flex flex-col md:row items-start md:items-end justify-between gap-8">
                        <div className="flex-grow">
                            <h1 className="text-4xl md:text-6xl font-heading font-black mb-4 tracking-tighter">
                                {t(`resume.tracks.${trackId}.title`)}
                            </h1>
                            <p className="text-xl text-accent-blue font-bold uppercase tracking-widest text-sm mb-6">
                                {trackId} {t("nav.resume")}
                            </p>
                            <p className="text-lg text-white/60 max-w-3xl border-l-2 border-accent-blue/30 pl-6 py-2 italic">
                                "{t(`resume.tracks.${trackId}.summary`)}"
                            </p>
                        </div>
                        <Button variant="primary" size="lg" className="w-full md:w-auto">
                            <Download className="mr-2 w-4 h-4" /> {t("hero.downloadResume")}
                        </Button>
                    </header>
                        ...
                        <div className="lg:col-span-2 space-y-12">
                            <section>
                                <div className="flex items-center space-x-3 mb-8">
                                    ...
                                    <h2 className="text-2xl font-heading font-bold uppercase tracking-widest text-sm">
                                        {t("resume.ui.professionalExperience")}
                                    </h2>
                                </div>
                                    ...
                                    {data.experience.map((exp: Experience, i: number) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col md:row items-start md:items-center justify-between mb-4 gap-2">
                                                <div>
                                                    <h3 className="text-xl font-heading font-bold text-white mb-1">
                                                        {exp.i18nKey ? t(`resume.tracks.${trackId}.experience.${exp.i18nKey}.role`) : exp.role}
                                                    </h3>
                                            ...
                                            <ul className="space-y-3">
                                                {(() => {
                                                    const desc = exp.i18nKey ? t(`resume.tracks.${trackId}.experience.${exp.i18nKey}.description`, { returnObjects: true }) : exp.description
                                                    return Array.isArray(desc) ? desc : exp.description
                                                })().map((desc: string, j: number) => (
                                    ...
                                    {data.education.map((edu: Education, i: number) => (
                                        <div key={i} className="glass-card p-8">
                                            <div className="flex flex-col md:row items-start md:items-center justify-between mb-4 gap-2">
                                                <div>
                                                    <h3 className="text-xl font-heading font-bold text-white mb-1">
                                                        {edu.institution.includes("University of Bristol") ? t("resume.shared.education.bristolDegree") : edu.degree}
                                                    </h3>
                                                    <p className="text-accent-purple font-medium">
                                                        {edu.institution.includes("University of Bristol") ? t("resume.shared.education.bristolInstitution") : edu.institution}
                                                    </p>
                                                </div>
                                                <div className="flex items-center text-white/40 text-sm bg-white/5 px-3 py-1 rounded-full border border-white/5">
                                                    <Calendar size={14} className="mr-2" />
                                            ...
                                            <p className="text-white/60 text-sm">
                                                {edu.institution.includes("University of Bristol") ? t("resume.shared.education.bristolDescription") : edu.description}
                                            </p>
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    {data.skills.map((cat: SkillCategory, i: number) => (
                                        <div key={i}>
                                            <h4 className="text-xs font-bold uppercase tracking-widest text-white/40 mb-4">
                                                {t(`resume.shared.skills.${cat.title.toLowerCase().replace(/[\/\s&-]+/g, '')}`)}
                                            </h4>
                                    ...
                                    {data.languages.map((lang: { name: string; level: string }, i: number) => (
                                        <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/5 group hover:border-accent-blue/50 transition-colors">
                                            <span className="font-bold text-white tracking-tight">
                                                {lang.name === "English" ? t("resume.shared.languages.english") :
                                                    lang.name === "Spanish" ? t("resume.shared.languages.spanish") :
                                                        lang.name === "French" ? t("resume.shared.languages.french") : lang.name}
                                            </span>
                                            <span className="text-xs font-black uppercase tracking-widest text-accent-blue bg-accent-blue/10 px-3 py-1 rounded-full">
                                                {lang.level === "Native" ? t("resume.shared.languages.native") :
                                                    lang.level === "Working Proficiency" ? t("resume.shared.languages.workingProficiency") : lang.level}
                                            </span>
                                        </div>
                                    ))}
                                </div>
