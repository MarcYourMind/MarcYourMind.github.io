/**
 * MOCK API & SIMULATION ENGINE for Serverless Demo
 */
class MockAPI {
    static async fetch(url, options = {}) {
        const path = url.split('?')[0];
        console.log(`[MockAPI] Fetching: ${path}`, options);
        ...
        if (path === '/status') {
            return {
                ok: true, json: async () => ({
                    ...window.MOCK_DATA.status,
                    agents: window.MOCK_DATA.agents
                })
            };
        }
        if (path === '/portfolio') {
            return { ok: true, json: async () => window.MOCK_DATA.portfolio };
        }
        if (path === '/trades') {
            return { ok: true, json: async () => window.MOCK_DATA.trades };
        }
        if (path === '/logs') {
            return { ok: true, json: async () => window.MOCK_DATA.logs };
        }
        if (path === '/equity') {
            return { ok: true, json: async () => window.MOCK_DATA.equity };
        }
        if (path === '/analysis/symbols') {
            const symbols = (window.MOCK_DATA.status && window.MOCK_DATA.status.config) ? window.MOCK_DATA.status.config.symbols : [];
        ...
        if (path.startsWith('/analysis/')) {
            ...
            return {
                ok: true, json: async () => ({
                    ...
                    market_regime: { "1h": window.MOCK_DATA.status.is_running ? "TRENDING" : "RANGING" },
                    market_structure: { "1h": { highs: "Higher", lows: "Higher", adx: 28.5 } },
                    market_data: { "1h": [{ Close: 65120, "Average Directional Index": 28.5 }] }
                })
            };
        }
        if (path.startsWith('/prompts/')) {
            const p = path.split('/').pop();
            return { ok: true, json: async () => window.MOCK_DATA.prompts[p] || { system: "N/A", user: "N/A" } };
        }
        if (path === '/start') {
            window.MOCK_DATA.status.is_running = true;
            MockAPI.addLog("GovernorAgent", "system_status", "System started manually by user.");
            return { ok: true, json: async () => ({ message: "Demo started" }) };
        }
        if (path === '/stop' || path === '/emergency-stop') {
            window.MOCK_DATA.status.is_running = false;
            MockAPI.addLog("GovernorAgent", "system_status", "System stopped manually by user.");
            return { ok: true, json: async () => ({ message: "Demo stopped" }) };
        }

        return { ok: false, status: 404, json: async () => ({ error: "Not found" }) };
    }

    static addLog(agent, type, msg) {
        window.MOCK_DATA.logs.unshift({
            timestamp: new Date().toISOString(),
            agent_name: agent,
            event_type: type,
            data: msg
        });
        if (window.MOCK_DATA.logs.length > 50) window.MOCK_DATA.logs.pop();
    }
}

// Override global fetch
window.originalFetch = window.fetch;
window.fetch = MockAPI.fetch;

// Simulation Loop
setInterval(() => {
    if (!window.MOCK_DATA.status || !window.MOCK_DATA.status.is_running) return;

    // 1. Update prices slightly
    window.MOCK_DATA.portfolio.forEach(p => {
        const change = (Math.random() - 0.495) * (p.price * 0.001);
        p.current_price += change;
        if (p.price > 0) {
            p.unrealized_pnl = (p.current_price - p.price) * p.amount * (p.side === 'buy' ? 1 : -1);
            p.unrealized_pnl_pct = (p.unrealized_pnl / (p.price * p.amount)) * 100;
        }
    });

    // 2. Randomly add logs
    if (Math.random() > 0.8) {
        const agents = ["StrategyAgent", "MarketDataAgent", "RiskAgent", "ExecutionAgent"];
        const agent = agents[Math.floor(Math.random() * agents.length)];
        const types = ["market_data", "signal_detected", "analysis_update"];
        const type = types[Math.floor(Math.random() * types.length)];
        const msg = type === "market_data" ? { symbol: "BTC/USDT", latest_close: window.MOCK_DATA.portfolio[0].current_price, candles: 100 } : "Neutral activity detected.";
        MockAPI.addLog(agent, type, msg);
    }

    // 3. Update equity history
    if (Math.random() > 0.95) {
        const lastEquity = window.MOCK_DATA.equity[window.MOCK_DATA.equity.length - 1].total_equity;
        window.MOCK_DATA.equity.push({
            timestamp: new Date().toISOString(),
            total_equity: lastEquity + (Math.random() - 0.4) * 50
        });
        if (window.MOCK_DATA.equity.length > 100) window.MOCK_DATA.equity.shift();
    }
}, 2000);
